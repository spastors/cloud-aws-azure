...
* **AWS WAF (Web Application Firewall):** Protege tus aplicaciones web de ataques comunes (Capa 7).
    * **Clave:** Bloquea **SQL Injection** y **Cross-Site Scripting (XSS)**. Puedes filtrar por direcciones IP.
* **AWS Shield:** Protección específica contra ataques **DDoS** (Denegación de Servicio Distribuido).
    * **Shield Standard:** Gratuito y activado por defecto para todos.
    * **Shield Advanced:** De pago, ofrece protección 24/7 y asistencia de expertos.
* **Amazon Macie:** Privacidad de datos en **S3**.
    * **Clave:** Usa Machine Learning para encontrar automáticamente **PII** (información de identificación personal como números de tarjeta o DNI) en tus cubos de S3.
* **Amazon GuardDuty:** Detección de amenazas inteligente.
    * **Clave:** Analiza logs (CloudTrail, VPC Flow Logs) para detectar actividad maliciosa como **minería de criptomonedas** o accesos desde IPs sospechosas.
* **Amazon Inspector:** Evaluador de vulnerabilidades para **EC2**.
    * **Clave:** Analiza tus instancias en busca de fallos de seguridad o desviaciones de las "mejores prácticas".

* **AWS Security Hub:** El **Panel de Control Central**.
    * **Función:** Reúne todas las alertas de seguridad de GuardDuty, Inspector, Macie y de terceros en un solo lugar.
    * **Clave:** Te da una "puntuación de seguridad" y te dice qué tan bien estás siguiendo las mejores prácticas.
* **AWS Firewall Manager:** Gestión centralizada de reglas.
    * **Función:** Si tienes 100 cuentas de AWS, no vas una por una configurando el WAF. Usas Firewall Manager para desplegar reglas de **WAF, Shield y Security Groups** en toda tu organización a la vez.

### Gestión Operativa, Gobernanza y Auditoría

Estos servicios registran qué pasa y cómo cambian las cosas.

* **AWS CloudTrail:** **¿Quién hizo qué?**
    * Registra todas las acciones (llamadas a la API) realizadas en tu cuenta.
    * **Uso:** Auditoría, cumplimiento y saber quién borró un recurso un martes a las 3 AM.
* **AWS Config:** **¿Cómo ha cambiado mi configuración?**
    * Guarda un historial de cómo estaban configurados tus recursos en el pasado.
    * **Uso:** Ver si alguien cambió el tipo de una instancia o si un bucket pasó de ser privado a público.
* **AWS Artifact:** Portal de autoservicio donde puedes descargar los informes de cumplimiento de AWS. Tu portal de **Cumplimiento**.
    * Es el lugar donde descargas documentos legales y certificaciones de AWS (ISO, SOC, PCI), es decir, sirve para demostrarle a auditores o reguladores que la infraestructura de AWS cumple con estándares internacionales como ISO, SOC, o PCI (para pagos con tarjeta).
    * **Uso:** Si un auditor te pide pruebas de que AWS es seguro, vas aquí.
* **AWS Audit Manager:** El **Recolector de Evidencia**.
    * **Función:** Ayuda a automatizar la recopilación de pruebas para auditorías (como la ISO o PCI). 
    * **Diferencia con Artifact:** En *Artifact* descargas el informe de AWS; en *Audit Manager* creas el informe de **tu propia empresa** recolectando evidencia de tus recursos.
* **AWS Systems Manager (SSM):** El **Cuchillo Suizo Operativo**.
    * **Función:** Permite gestionar tus instancias EC2 a gran escala (parchear 500 servidores a la vez, ejecutar comandos de terminal sin entrar por SSH).
    * **Clave:** Incluye el **Parameter Store** (para guardar configuraciones) y el **Patch Manager**.
...
* **AWS KMS (Key Management Service):** Servicio para crear y controlar las llaves de cifrado que protegen tus datos.
    * Se usa para cifrar datos **"En Reposo"** (At Rest), es decir, guardados en el disco (S3, EBS, RDS).
* **AWS Secrets Manager:** El **Llavero Inteligente**.
    * **Función:** Guarda contraseñas de bases de datos, llaves de API y secretos. 
    * **Clave:** Su gran superpoder es la **Rotación Automática**. Puede cambiar la contraseña de tu base de datos RDS cada 30 días automáticamente sin que tú hagas nada.
* **Cifrado "En Tránsito" (In Transit):**
    * Se logra mediante **TLS/SSL**. Es la protección de los datos mientras viajan por internet (del usuario al servidor).
    * Servicio relacionado: **AWS Certificate Manager (ACM)** para gestionar esos certificados SSL.

### Tabla Comparativa: "No te confundas"

| Si la pregunta menciona... | El servicio es... | Diferenciador clave (El "Truco") |
| :--- | :--- | :--- |
| **Ataques Web** (SQLi, XSS, IPs malas) | **AWS WAF** | Actúa en la **Capa 7** (Aplicación). |
| **Ataque DDoS** (Inundación de tráfico) | **AWS Shield** | **Standard** (gratis) vs **Advanced** (pago). |
| **Datos Personales (PII)** en S3 | **Amazon Macie** | Usa Machine Learning para la **privacidad**. |
| **Actividad Maliciosa** / Minería Cripto | **Amazon GuardDuty** | Detecta **amenazas** inteligentes. |
| **Vulnerabilidades** en EC2/Software | **Amazon Inspector** | Busca **fallos** de seguridad internos. |
| **Panel Central** de alertas | **AWS Security Hub** | Reúne a todos los anteriores en un **dashboard**. |
| Gestión de reglas en **muchas cuentas** | **AWS Firewall Manager** | Despliega reglas de WAF/Shield **a escala**. |
| **¿Quién hizo qué?** (Llamadas API) | **AWS CloudTrail** | Es el "Gran Hermano" para **Auditoría**. |
| **¿Cómo cambió esto?** (Historial) | **AWS Config** | Guarda el **estado pasado** de los recursos. |
| **Métricas** (CPU, RAM) y Alarmas | **Amazon CloudWatch** | Enfocado en **Rendimiento** y Salud. |
| **Informes de AWS** (ISO, SOC, PCI) | **AWS Artifact** | Descargas los "papeles" **de AWS**. |
| **Recolección de evidencia** propia | **AWS Audit Manager** | Generas informes de cumplimiento **tuyos**. |
| **Parchear** 100 servidores a la vez | **AWS Systems Manager (SSM)** | Operaciones y comandos **masivos**. |
| **Llaves** de cifrado (Reposo) | **AWS KMS** | Gestiona las "matemáticas" del **cifrado**. |
| **Rotación** de contraseñas de DB | **AWS Secrets Manager** | Cambia el **secreto** automáticamente. |
| **Certificados SSL/TLS** (Tránsito) | **AWS Certificate Manager (ACM)** | Gestiona el candado de **HTTPS**. |


### Un par de consejos "Ninja" para el examen:

* **Artifact vs. Audit Manager:** Si la pregunta dice "necesito el certificado ISO de AWS", es **Artifact**. Si dice "necesito recolectar pruebas de mis recursos para una auditoría", es **Audit Manager**.
...
Es tu sección aislada de forma lógica en la nube de AWS donde lanzas tus recursos. Es tu "parcela" privada.

* **Subredes (Subnets)**:
    * **Pública**: Tiene acceso directo a internet gracias a un **Internet Gateway (IGW)**.
    * **Privada**: No tiene acceso directo a internet (necesita un NAT Gateway para "salir" sin ser vista).

* **¿Cómo recordarlo?**: Imagina un **edificio de oficinas**. La VPC es el edificio completo. La subred pública es la recepción (donde entra cualquiera) y la privada son las cajas fuertes del sótano.

### Seguridad de Red: Grupos de Seguridad vs. NACL

| Característica | **Security Groups (SG)** | **Network ACL (NACL)** |
| :--- | :--- | :--- |
| **Nivel** | **Instancia** (Capa de Host) | **Subred** (Capa de Red) |
| **Estado** | **Stateful** (Con estado) | **Stateless** (Sin estado) |
| **Reglas** | Solo reglas de "Permitir" | Reglas de "Permitir" y "Denegar" |
| **¿Qué hace?** | Si dejas entrar algo, sale solo. | Tienes que pedir permiso para entrar Y para salir. |

* **¿Cómo recordarlo?**: 
    * **SG**: Es el **portero de una discoteca** que te da un sello. Como ya tienes el sello, te deja salir y volver a entrar sin preguntar.
    * **NACL**: Es la **aduana de un aeropuerto**. Revisan tu pasaporte al entrar y *vuelven* a revisarlo al salir. No se fían de nadie.
* **Cómo cae en el examen**: "Seguridad a nivel de instancia" (SG) o "Capa de seguridad adicional para la subred" (NACL).
* **Confusión estrella**: Olvidar que el SG es *Stateful*. En el examen, si algo entra por el SG, la respuesta siempre es que puede salir automáticamente.

### Conexión: VPN vs. Direct Connect (DX)

* **AWS VPN**: 
    * **Qué es**: Conexión cifrada a través de la **internet pública**.
    * **Examen**: "Rápida de configurar", "Bajo coste", "Usa internet".
* **AWS Direct Connect**:
    * **Qué es**: Una **línea física dedicada** (fibra óptica) desde tu oficina a AWS. No pasa por internet.
    * **Examen**: "Ancho de banda predecible", "Máxima seguridad", "Latencia muy baja". Es cara y tarda semanas en instalarse.
